...
GameInfo_t create_banner() {
  ...
  tmp.score = 0;
  FILE *tmp_file;
  tmp_file = fopen("./high_score.txt", "r");
  char buffer[20];
...
int **new_field() {
  int status = 1;
  int n;
  int **field = calloc(25, sizeof(int *));
  if (field != NULL) {
    for (int i = 0; i < 25 && status; ++i) {
      field[i] = calloc(16, sizeof(int));
      if (field[i] == NULL) {
        status = 0;
        n = i;
      }
...
int **new_figure() {
  int status = 1;
  int n;
  int **figure = calloc(4, sizeof(int *));
  if (figure != NULL) {
    for (int i = 0; i < 4 && status; ++i) {
      figure[i] = calloc(4, sizeof(int));
      if (figure[i] == NULL) {
        status = 0;
        n = i;
      }
...
void write_high_score(int high_score) {
  char str[10];
  snprintf(str, sizeof str, "%d", high_score);
  FILE *tmp_file;
  tmp_file = fopen("./high_score.txt", "w");
  fputs(str, tmp_file);
  fclose(tmp_file);
}